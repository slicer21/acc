    ...
    try {
        ...
        switch ($_GET['type']) {
            ...
            case 'balance_sheet':
                ...
                $sheet->setCellValue("B{$row}", (float)($total_equity + $net_income))->getStyle("B{$row}")->getFont()->setBold(true);
                $sheet->getStyle("B4:B{$row}")->getNumberFormat()->setFormatCode('#,##0.00');
                $row += 2;

                // Total Liabilities & Equity
                $sheet->setCellValue("A{$row}", 'TOTAL LIABILITIES & EQUITY')->getStyle("A{$row}")->getFont()->setBold(true);
                $sheet->setCellValue("B{$row}", (float)($total_liabilities + $total_equity + $net_income))->getStyle("B{$row}")->getFont()->setBold(true);
                $sheet->getStyle("B{$row}")->getNumberFormat()->setFormatCode('#,##0.00');
